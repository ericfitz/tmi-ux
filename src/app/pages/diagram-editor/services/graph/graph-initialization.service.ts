        ...
        if (originalInstallListeners) {
          this.graph.cellEditor.installListeners = function (inputElement: HTMLElement) {
            ...
            if (inputElement) {
              ...
              const inputEl = inputElement as HTMLElement & {
                _mxGraphKeydownHandler?: (e: KeyboardEvent) => void;
              };
            ...
            this.graph.cellEditor.startEditing = function () {
              ...
              try {
                if (this.editingCell && this.graph && this.graph.selectionHandler) {
                  const handler =
                    this.graph.selectionHandler.handlers &&
                  ...
                  if (handler && handler.labelShape) {
                    ...
                    handler.labelShape._wasVisible =
                      handler.labelShape.node &&
                    ...
                    try {
                      ...
                      if (state && state.text && state.text.boundingBox) {
                        ...
                        if (handler && handler.labelShape) {
                          ...
                          handler.labelShape._pendingX =
                            bbox.x + bbox.width / 2 - handler.labelShape.bounds.width / 2;
                          handler.labelShape._pendingY =
                            bbox.y - handler.labelShape.bounds.height - 2;
                        }
                      }
                    } catch (e) {
                ...
                if (this.editingCell && this.graph && this.graph.selectionHandler) {
                  ...
                  if (handler && handler.labelShape) {
                    ...
                    if (handler.labelShape.node && shouldBeVisible) {
                      ...
                      if (
                        handler.labelShape._pendingX !== undefined &&
