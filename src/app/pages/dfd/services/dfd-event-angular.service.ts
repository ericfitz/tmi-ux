...
import { DfdNodeAngularService } from './dfd-node-angular.service';
...
/**
 * Service for managing events in the DFD component with Angular shapes
 */
@Injectable({
  providedIn: 'root',
})
export class DfdEventAngularService {
    ...
    graph.on('node:change:parent', ({ node, current, previous }) => {
      if (current && !previous) {
        ...
        if (this.nodeService.isDfdNode(node)) {
          // Update the node data to indicate it's embedded
  ...
  private setupNodeClickEvents(graph: Graph): void {
    graph.on('node:click', ({ cell, e }) => {
      ...
      // Add a custom attribute to the node to indicate its shape
      const shape = cell.shape;
      if (shape.includes('process')) {
