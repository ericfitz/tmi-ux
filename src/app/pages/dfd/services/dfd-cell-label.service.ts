...
/**
 * Interface for node data change events (for history integration)
 */
...
export class DfdCellLabelService {
  ...
  private _nodeDataChanged$ = new Subject<NodeDataChangeEvent>();
  ...
  /**
   * Observable for node data change events (for history integration)
   */
  ...
  setCellLabel(cell: Cell, text: string): boolean {
    ...
    // For nodes, emit data change event for history integration
    if (cell.isNode()) {
      this.emitNodeDataChangeForHistory(cell.id, oldLabel, text);
    }
  ...
  /**
   * Emit node data change event for history system integration
   */
  private emitNodeDataChangeForHistory(nodeId: string, oldLabel: string, newLabel: string): void {
    const oldData = { label: oldLabel };
    const newData = { label: newLabel };

    this.logger.info('[CellLabelService] Emitting node data change for history integration', {
      nodeId,
      oldData,
      newData,
    });
