...
export class DfdService {
  ...
  private logHistoryChanges(
    ...
    after.forEach(afterCell => {
      ...
      if (beforeCell) {
        ...
        if (afterCell.position && beforeCell.position && 
        ...
        // Check size changes
        if (afterCell.size && beforeCell.size && 
        ...
        const beforeLabel = beforeAttrs && 'label' in beforeAttrs ? beforeAttrs['label'] : undefined;
        const afterLabel = afterAttrs && 'label' in afterAttrs ? afterAttrs['label'] : undefined;
        
        if (beforeLabel && afterLabel && 
            typeof beforeLabel === 'object' && typeof afterLabel === 'object' &&
            'text' in beforeLabel && 'text' in afterLabel &&
        ...
        if (JSON.stringify(beforeCell.data) !== JSON.stringify(afterCell.data)) {
          ...
          if (typeof beforeCell.data === 'object' && typeof afterCell.data === 'object' &&
              beforeCell.data !== null && afterCell.data !== null) {
            ...
            if ('label' in beforeData && 'label' in afterData && 
        ...
        if (afterCell.points && beforeCell.points && 
  ...
  private setupHistoryStateListener(): void {
    ...
    history.on('change', () => {
      ...
      this.logger.debug(
        `History state changed: canUndo=${canUndoValue}, canRedo=${canRedoValue}`,
      );
    });
