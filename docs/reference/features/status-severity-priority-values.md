# Plan for implementing rich values for status, severity and priority

## Plan notes

Localization files are json files. Renaming, moving or deleting a key or section is a 1-line operation with jq. Make a script like scripts/dedup-localization.sh that applies every such change to all language files, rather than individually editing each file.

## High level plan

1. Create the script file that uses jq to rename/move the existing common.severity\* keys, in every supported language file.
2. Modify the script to also remove the empty "footer" section
3. Find each of the strings to be removed, in the code, and update their references.
4. Execute the script to modify the localization files and then run check-i18n and fix any issues.
5. Add keys for the 4 groups of strings listed in the tables below.

- the key names will use the stated prefix from the description above the table, and then append the key name from the first column of the table, e.g. threatModels.status.0 or threatEditor.threatSeverity.5

6. Using the output of check-i18n, complete any translations needed for any keys in all other languages.

## Keys and sections to remove or replace

- keys
  - rename/move common.severityCritical --> threatEditor.threatSeverity.0
  - rename/move common.severityHigh --> threatEditor.threatSeverity.1
  - rename/move common.severityLow --> threatEditor.threatSeverity.3
  - rename/move common.severityMedium --> threatEditor.threatSeverity.2
  - rename/move common.severityUnknown --> threatEditor.threatSeverity.5
- sections
...
| key name | en-US key value         | en-US tooltip key value value                                                                            |
| -------- | ----------------------- | -------------------------------------------------------------------------------------------------------- |
...
| key name | en-US key value        | en-US description value                                                                                                            |
| -------- | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
...
| 5        | Unknown         | Threat severity has not yet been assessed.                                                                              |
