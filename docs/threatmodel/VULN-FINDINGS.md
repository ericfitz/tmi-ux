...
<!-- REMEDIATED (2026-08-01 review): fixed in 0ec083bf (renderer.link output now
DOMPurify-sanitized) and hardened in 90bfd732 / #730 (sanitizeMarkdownHtml with the
shared MARKDOWN_DOMPURIFY_CONFIG is now registered as the ngx-markdown SANITIZE
chokepoint, so no renderer can bypass it — this also closes root-cause F-014).
E2E regression coverage: e2e/tests/workflows/markdown-sanitization.spec.ts with
payloads in e2e/helpers/markdown-xss.ts. The description below records the
pre-fix state. -->
...
**Fix:** add `helmet`/explicit middleware emitting CSP (`frame-ancestors 'none'`), `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy`, `Permissions-Policy`, and HSTS. _Caveat: a fronting CDN/proxy may add these in production — open question in the threat model._
