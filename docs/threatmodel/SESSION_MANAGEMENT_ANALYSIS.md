...
<!-- OUTDATED (2026-08-01 review): this analysis describes the pre-HttpOnly-cookie design
(JWTs in encrypted localStorage). The app has since migrated to HttpOnly-cookie sessions;
THREAT_MODEL.md explicitly reconciles this document as describing the superseded design.
Kept for historical context — do not use as a description of current session handling. -->
...
if (currentToken.refreshToken === response.refresh_token) {
  this.logger.debug('Refresh token was not rotated by server (same token returned).');
}
```
