...
it('should enforce DFD connection validation rules', () => {
  ...
  cy.dfdAttemptConnection('User', 'Database').should('be.rejected');
...
describe('Export and Persistence', () => {
  it('should export diagrams in multiple formats', () => {
    ...
    // Test PNG export
    cy.dfdExportDiagram('png').should('have.property', 'type', 'image/png');

    // Test JPEG export
    cy.dfdExportDiagram('jpeg').should('have.property', 'type', 'image/jpeg');
  });
});
```
...
Cypress.Commands.add('dfdVerifyCleanState', identifier => {
  ...
  // Default styling
  cy.get(selector).find('path, rect, ellipse').should('have.css', 'stroke-width', '2px');
});
