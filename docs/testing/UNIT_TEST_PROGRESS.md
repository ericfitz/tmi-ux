...
**Current Status:** 10/40 services complete (25% of gap)
...
## Remaining Services (30/40)

### Priority 2: TM Services (8 services)
**Status:** Not started
**Complexity:** Medium
**Location:** `src/app/pages/tm/services/`

- [ ] `threat-model-authorization.service.ts` - Permission checking and role management
- [ ] `threat-model-report.service.ts` - Report generation
- [ ] `import-orchestrator.service.ts` - Import coordination
- [ ] `id-translation.service.ts` - ID mapping during import
- [ ] `readonly-field-filter.service.ts` - Field filtering
- [ ] `reference-rewriter.service.ts` - Reference rewriting during import
- [ ] `authorization-prepare.service.ts` - Authorization preparation
- [ ] `provider-adapter.service.ts` - Provider abstraction
